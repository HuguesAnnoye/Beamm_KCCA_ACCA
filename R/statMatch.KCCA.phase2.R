#' @title statMatch.KCCA.phase2
#' @description Statistical matching (phase2) using KCCA
...
statMatch.KCCA.phase2 <- function(df.rec, df.don, don.weights, names.CV, names.NCV, zero.constraints, opts) {
  ...
  if ((opts$P2$n_combs) * (opts$P2$n_h) != 1) {
    if (isTRUE(opts$par)) {
      ...
      tune <- statMatch.KCCA.phase2.tuning.par(df.don, names.CV, names.NCV, don.weights, zero.constraints, opts)
      } else {
      tune <- statMatch.KCCA.phase2.tuning.par2(df.don, names.CV, names.NCV, don.weights, zero.constraints, opts)
      }
    } else {
    tune <- statMatch.KCCA.phase2.tuning(df.don, names.CV, names.NCV, don.weights, zero.constraints, opts)
    }
  } else {
