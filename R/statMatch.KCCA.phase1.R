...
statMatch.KCCA.phase1 <- function(df.rec, df.don, don.weights, names.CV, names.NCV, opts) {
  ...
  if (length(names.CV.cat) > 0 & length(names.NCV.cat) > 0) {
    ...
    if ((opts$P1$n_combs) * (opts$P1$n_h) != 1) {
      if (isTRUE(opts$par)) {
        ...
        tune <- statMatch.KCCA.phase1.tuning.par(df.don, names.CV, names.NCV.cat, don.weights, opts)
        } else {
          tune <- statMatch.KCCA.phase1.tuning.par2(df.don, names.CV, names.NCV.cat, don.weights, opts)
        }
      } else {
      tune <- statMatch.KCCA.phase1.tuning(df.don, names.CV, names.NCV.cat, don.weights, opts)
      }
    } else {
