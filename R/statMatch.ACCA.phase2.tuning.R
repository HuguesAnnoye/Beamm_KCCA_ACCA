#' @title statMatch.ACCA.phase2.tuning
...
#' @noRd
#'
statMatch.ACCA.phase2.tuning <- function(df.don, names.CV, names.NCV, don.weights, zero.constraints, opts) {
    ...
    for (fold in seq_len(opts$n_fold)) {
      ...
      if (fit_autoencoder) {
        ...
        autoencoder_X <- statMatch.ACCA.autoencoder(
          n_var = n_X,
          n_lat = opts$P2$hpar$lat_X[i],
          n_hidden_layers = opts$P2$nlayers,
          n_units = as.numeric((opts$P2$hpar %>% select(starts_with("units_X_")))[i,]),
          penL1 = opts$P2$hpar$penL1_X[i],
          penL2 = opts$P2$hpar$penL2_X[i],
          lr = opts$P2$hpar$lr_X[i]
        )
        # Initialize the autoencoder for the non-common variables
        autoencoder_Y <- statMatch.ACCA.autoencoder(
          n_var = n_Y,
          n_lat = opts$P2$hpar$lat_Y[i],
          n_hidden_layers = opts$P2$nlayers,
          n_units = as.numeric((opts$P2$hpar %>% select(starts_with("units_Y_")))[i,]),
          penL1 = opts$P2$hpar$penL1_Y[i],
          penL2 = opts$P2$hpar$penL2_Y[i],
          lr = opts$P2$hpar$lr_Y[i]
        )
        ...
        cca_model[[fold]] <- statMatch.ACCA.phase2.cca(autoencoder_X,
                                              autoencoder_Y,
      ...
      # Compute predictions
      pred <- statMatch.ACCA.phase2.predict(cca_model[[fold]],
                                            opts$P2$hpar$h[i],
                                            opts$P2$hpar$d[i],
                                            input$weights[[fold]],
                                            opts, zero.constraints)
