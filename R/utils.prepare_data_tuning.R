prepare_data_tuning <- function(df.don, names.CV, names.NCV, don.weights, opts, method = c("all", "MLP_phase1", "CCA"),
  ...
  for (fold in seq_len(opts$n_fold)) {
    ...
    if (opts$scaling != "no")
      out$raw_data[[fold]] <- statMatch.scale(data = out$raw_data[[fold]],
                                              wt = out$weights[[fold]],
                                              method = opts$scaling,
