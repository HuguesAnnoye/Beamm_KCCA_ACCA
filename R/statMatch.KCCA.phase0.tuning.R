...
statMatch.KCCA.phase0.tuning <- function(df.don, names.CV, names.NCV, names.NCV_cat, names.NCV_cont, don.weights, zero.constraints, opts) {
  ...
  count_h <- 0
  results_for <- NULL
  for (j in seq_len(opts$P0$n_combs)) {
    fit <- list()
    ...
    # return(results2)
    results_for <- rbind(results_for, results2)
  }
