...
statMatch.scale <- function(data, wt, method = c("z-score", "min-max"), scale_NCV = T) {
  method <- match.arg(method)
  if (method == "z-score") {
    data$mtx.don.CV.raw <- scale_mtx(data$mtx.don.CV.raw, wt)
    if (scale_NCV) data$mtx.don.NCV.raw <- scale_mtx(data$mtx.don.NCV.raw, wt)
    ...
    if (scale_NCV) data$mtx.don.NCV.raw <- normalize_mtx(data$mtx.don.NCV.raw)
