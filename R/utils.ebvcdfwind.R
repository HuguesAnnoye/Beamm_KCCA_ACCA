...
ebvcdfwind <- function(x, wt, ..., data) {
  ...
  sf <- function(x, wt) {
    sf <- .THIS()
    .cbv.evalind(sf, .ebvcdf.eval.ext.wtind, x)
  }

  new("EBVCDFWIND", sf,
      .class.info = .get.infow("EBVCDFWIND"),
...
setClass ("EBVW", contains="SBV",
          slots = list (
            ...
            wt="vector",
            data="matrix") )
setClass ("EBVCDFWIND", contains = c ("EBVW", "SBV", "BV", "CDF", "function") )

.ebvcdf.eval.ext.wtind <- function(sf, x) {
  n0 <- sf@n
  x0 <- sf@data
  w0 <- sf@wt
  ...
  for (i in seq_len(n))
  {
    ...
    z1 [i] <- sum(w0* Ix_1) / n0
    for (j in 2:ncol(x0)) {
      #assign(paste0("Ix_",j),(x0[,j] <= x[i,j]))
      Ix <- (x0[,j] <= x[i,j])
      z1 [i] <- z1 [i] * sum(w0* Ix) / n0
    }
    #z1 [i] <- sum(w0* Ix) / n0
  }
  z1
...
.class.infow = matrix (c (
  ...
  "EBVCDFWIND",  "Multivariate Empirical Distribution Function supposing the independence with weight",         "Fh (x)"
  ),, 3, byrow=TRUE)

.cbv.evalind = function (sf, evalf, x)
{
#x = as.numeric (x)
#y = as.numeric (y)
#z = as.numeric (z)
...
.val.finiteind = function (x, n=ncol (x))
